...
module.exports = function(pb) {
     ...
     */
    function PageServiceEn(context){
        if (!util.isObject(context)) {
            context = {};
        }

        context.type = TYPE;
        PageServiceEn.super_.call(this, context);
    }
    util.inherits(PageServiceEn, ContentObjectService);
     ...
     * @return {Object}
     */
    PageServiceEn.prototype.getRenderOptions = function(options, isMultiple) {
     ...
     * @return {PageRenderer}
     */
    PageServiceEn.prototype.getRenderer = function() {
        return new pb.PageRendererEn();
    };
     ...
     */
    PageServiceEn.prototype.getTopicsForContent = function(content) {
     ...
     * @param {PageServiceEn} context.service An instance of the service that triggered
     * the event that called this handler
     * @param {Function} cb A callback that takes a single parameter: an error if occurred
     */
    PageServiceEn.format = function(context, cb) {
        ...
        dto.page_layout_en = BaseObjectService.sanitize(dto.page_layout_en, BaseObjectService.getContentSanitizationRules());
     ...
     * @method
     * @param {Object} context
     * @param {PageServiceEn} service An instance of the service that triggered
     * the event that called this handler
     * @param {Function} cb A callback that takes a single parameter: an error if occurred
     */
    PageServiceEn.merge = function(context, cb) {
        var dto = context.data;
        ...
        obj.page_media = dto.page_media_en;
        ...
        obj.page_layout_en = dto.page_layout_en;
     ...
     * @param {Function} cb A callback that takes a single parameter: an error if occurred
     */
    PageServiceEn.validate = function(context, cb) {
     ...
     * @param {Object} where
     */
    PageServiceEn.setSectionClause = function(where, sectionId) {
     ...
     * @param {Object} where
     */
    PageServiceEn.setTopicClause = function(where, topicId) {
        where.page_topics = topicId + '';
    };

    //Event Registries
    BaseObjectService.on(TYPE + '.' + BaseObjectService.FORMAT, PageServiceEn.format);
    BaseObjectService.on(TYPE + '.' + BaseObjectService.MERGE, PageServiceEn.merge);
    BaseObjectService.on(TYPE + '.' + BaseObjectService.VALIDATE, PageServiceEn.validate);

    return PageServiceEn;
};